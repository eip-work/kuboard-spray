...
func SyncEtcdConfigActions(c *gin.Context) {
	...
	postExec := func(status command.ExecuteExitStatus) (string, error) {
		...
		if success {
			...
			PostProcessInventory(req.Cluster, "sync_etcd_config")
		} else {
	...
	cmd := command.Execute{
		...
		Args: func(execute_dir string) []string {
			...
			playbook := common.MapGet(resourcePackage, "data.supported_playbooks.sync_etcd_config").(string)
			result := []string{"-i", execute_dir + "/inventory.yaml", playbook, "--fork", strconv.Itoa(req.Fork), "--tags", "etcd", "--limit", "kube_control_plane,etcd"}
			if req.ExcludeNodes != "" {
