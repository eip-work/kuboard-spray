...
func DownloadBinaries(c *gin.Context) {
	...
	postExec := func(status command.ExecuteExitStatus) (string, error) {
		...
		for _, nodeStatus := range status.NodeStatus {
			...
			if nodeStatus.Changed != "0" && nodeStatus.Unreachable == "0" && nodeStatus.Failed == "0" {
				common.MapDelete(clusterMetadata.Inventory, "all.hosts."+nodeStatus.NodeName+".pangeecluster_require_download")
				count++
	...
	playbook := common.MapGet(resourcePackage, "data.supported_playbooks.download_binaries").(string)
