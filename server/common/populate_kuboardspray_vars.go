...
func PopulateKuboardSprayVars(inventory map[string]interface{}, ownerType, ownerName string) {
	...
	MapSet(inventory, "all.vars.kuboardspray_ssh_args", "-o ConnectionAttempts=100 -o UserKnownHostsFile=/dev/null -F /dev/null")
	MapSet(inventory, "all.vars.kuboardspray_ssh_multiplexing", "-o ControlMaster=auto -o ControlPersist=30m -o ControlPath={{ kuboardspray_cluster_dir }}/ansible-{{ansible_user}}@{{ansible_host}}:{{ansible_port}}")
}
