...
<template>
  <ConfigSection v-model:enabled="enableSsh" label="SSH" :description="description" disabled>
    ...
    <FieldString :holder="holder" fieldName="ansible_port" :prop="`all.hosts.${nodeName}`"
      ...
      :required="!cluster.inventory.all.children.managed.children.k8s_cluster.vars.ansible_port"></FieldString>
...
export default {
  ...
  computed: {
    ...
    ansible_become: {
      get () {
        ...
        return this.cluster.inventory.all.children.managed.children.k8s_cluster.vars.ansible_become 
      },
      set (v) {
  ...
  methods: {
    placeholder(fieldName) {
      let default_value = this.cluster.inventory.all.children.managed.children.k8s_cluster.vars[fieldName]
