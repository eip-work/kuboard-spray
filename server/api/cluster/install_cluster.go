...
func InstallCluster(c *gin.Context) {
	...
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.kube_version", common.MapGet(resourcePackage, "kubernetes.kube_version"))
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.image_arch", common.MapGet(resourcePackage, "kubernetes.image_arch"))
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.gcr_image_repo", common.MapGet(resourcePackage, "kubernetes.gcr_image_repo"))
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.kube_image_repo", common.MapGet(resourcePackage, "kubernetes.kube_image_repo"))
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.container_manager", common.MapGet(resourcePackage, "docker_engine.container_manager"))
	dockerVersion := common.MapGet(resourcePackage, "docker_engine.target").(string)
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars."+dockerVersion, common.MapGet(resourcePackage, "docker_engine.version"))
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.container_manager", common.MapGet(resourcePackage, "docker_engine.container_manager"))
	common.MapSet(inventory, "all.children.managed.children.etcd.vars.etcd_version", common.MapGet(resourcePackage, "etcd.etcd_version"))
	common.MapSet(inventory, "all.children.managed.children.etcd.vars.etcd_deployment_type", common.MapGet(resourcePackage, "etcd.etcd_deployment_type"))
	...
	for _, d := range dependencies {
		...
		common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars."+field, version)
	}

	cni := common.MapGet(inventory, "all.children.managed.children.k8s_cluster.vars.kube_network_plugin").(string)
	...
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars."+cni_dependency["target"].(string), cni_dependency["version"])

	addons := resourcePackage["addons"].([]interface{})
	for _, a := range addons {
		addon := a.(map[string]interface{})
		enabled := common.MapGet(inventory, "all.children.managed.children.k8s_cluster.vars."+addon["target"].(string))
		if enabled == true {
			for key, value := range addon["params"].(map[string]interface{}) {
				common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars."+key, value)
			}
		}
	}
	...
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.disable_service_firewall", true)
	common.MapSet(inventory, "all.children.managed.children.etcd.vars.disable_service_firewall", true)
	common.MapSet(inventory, "all.children.managed.children.k8s_cluster.vars.ansible_python_interpreter", "/usr/bin/python3")
